...
class PokerGame:
    ...
    def start_new_hand(self):
        ...
        for i, player in enumerate(self.players):
            ...
            self.has_played[i] = False if player.status in [PlayerStatus.ACTIVE] else True
    ...
    def player_action(self, action: PlayerAction, amount: int = 0) -> bool:
        ...
        if action in [PlayerAction.BET, PlayerAction.RAISE]:
            ...
            if amount <= min_amount:
                print(f"Minimum {action.value} should be more than {min_amount}")
                return False
    ...
    def display_game_state(self):
        ...
        # from visualizer import update_gui
        # update_gui(self)
    ...
    def get_player_input(self) -> bool:
        ...
        initial_action_history = self.action_history.copy()
        ...
        if(initial_stack != final_stack):
          print(f"{player.name} has tampered with its stack")
          return False
        
        if(initial_action_history != final_action_history):
          print(f"{player.name} has changed the action history")
